(() => {
  ...
  function solutionMarkup(q){return `<div class="math-card"><div class="math-label">Formula</div><div class="formula">${q.formula||'Formula unavailable'}</div><div class="math-label equation-label">Using this question's numbers</div><div class="equation">${String(q.solution||'Worked equation unavailable').replace(/\n/g,'<br>')}</div></div><div class="classroom-final-answer"><strong>Final answer:</strong> ${q.answer}${q.unit?` ${q.unit}`:''}</div>`}
