export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && path === '/api/instructor-quizzes') {
      ...
      return new Response(raw || JSON.stringify({ version: 1, quizzes: [] }), { headers: cors });
    }

    if (request.method === 'GET' && path.startsWith('/api/instructor-quizzes/')) {
      const id = decodeURIComponent(path.split('/').pop());
      const raw = await env.INSTRUCTOR_QUIZZES.get(`quiz:${id}`);
      if (!raw) return new Response(JSON.stringify({ error: 'Quiz not found' }), { status: 404, headers: cors });
      return new Response(raw, { headers: cors });
    }

    if (request.method === 'POST' && path === '/api/instructor-quizzes') {
      const accessUser = request.headers.get('cf-access-authenticated-user-email');
      if (!accessUser) return new Response(JSON.stringify({ error: 'Cloudflare Access authentication required' }), { status: 401, headers: cors });
      ...
      if (!quiz || !quiz.id || !quiz.title || !Array.isArray(quiz.questions) || !quiz.questions.length) {
        return new Response(JSON.stringify({ error: 'Quiz id, title, and questions are required' }), { status: 400, headers: cors });
      }
      ...
      return new Response(JSON.stringify({ ok: true, id: frozen.id }), { status: 201, headers: cors });
    }
