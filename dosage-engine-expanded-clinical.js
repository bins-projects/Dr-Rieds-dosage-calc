(() => {
  ...
  function pedsSafe(d){
    ...
    const minMl=pedsVolume(min,c.have,c.qty),maxMl=pedsVolume(max,c.have,c.qty),target=pick(['low','safe','high']);
    let order;if(target==='low')order=Math.max(5,Math.round((min-Math.max(5,min*.15))/5)*5);else if(target==='high')order=Math.round((max+Math.max(5,max*.15))/5)*5;else order=Math.round(randDec(min,max,2)/5)*5;
    const safety=order<min?'too low':order>max?'too high':'safe',ml=pedsVolume(order,c.have,c.qty),rangeText=perDose?`${c.minDay}–${c.maxDay} mg/kg/dose`:`${c.minDay}–${c.maxDay} mg/kg/day in ${c.doses} equal doses`,safetyLabel=safety==='safe'?'Safe':safety==='too low'?'Too low':'Too high';
    ...
    const volumeRatio=pedsRound(order/c.have);
    return q('peds-safe-range','Pediatric safe-dose range',`A child weighs ${kg} kg and is being treated for ${c.context}. The provider orders ${c.drug} ${order} mg ${c.route}. Recommended: ${rangeText}${c.maxDaily?`, maximum ${c.maxDaily} mg/day`:''}. Available: ${c.have} mg/${c.qty} mL. Calculate the volume and determine whether the order is safe, too low, or too high.`,safety,'',perDose?'mg/kg/dose × kg = safe mg/dose range':'mg/kg/day × kg ÷ doses/day = safe mg/dose range','Round each completed pediatric calculation to the nearest hundredth and carry that result into the next step.',`Minimum: ${minWork}.\nMaximum: ${maxWork}.\nSafe dose range: ${min.toFixed(2)}–${max.toFixed(2)} mg/dose.\nSafe volume range: ${minMl.toFixed(2)}–${maxMl.toFixed(2)} mL/dose.\nOrdered dose: ${order} ÷ ${c.have} = ${volumeRatio.toFixed(2)}; ${volumeRatio.toFixed(2)} × ${c.qty} = ${ml.toFixed(2)} mL.\nFinal answer: ${ml.toFixed(2)} mL — ${safetyLabel}.`,.006,{drug:c.drug,context:c.context,kg,order,route:c.route,doses:c.doses,minDay:c.minDay,maxDay:c.maxDay,doseKind:c.doseKind,have:c.have,qty:c.qty,min,max,minRaw:min,maxRaw:max,minDaily,maxDailyRounded:maxDaily,minMl,maxMl,volumeRatio,ml,safety,maxDaily:c.maxDaily})
  }
