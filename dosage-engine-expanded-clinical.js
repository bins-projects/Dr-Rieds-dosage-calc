(() => {
  ...
  function pedsSafe(d){const c=pick(safeCases),kg=d==='easy'?Math.round(randDec(c.minKg,c.maxKg,0)):randDec(c.minKg,c.maxKg,1),perDose=c.doseKind==='per-dose',minRaw=perDose?kg*c.minDay:kg*c.minDay/c.doses,maxWeightRaw=perDose?kg*c.maxDay:kg*c.maxDay/c.doses,maxRaw=c.maxDaily?Math.min(maxWeightRaw,c.maxDaily/c.doses):maxWeightRaw,min=round(minRaw,1),max=round(maxRaw,1),minMl=finalRound(minRaw*c.qty/c.have),maxMl=finalRound(maxRaw*c.qty/c.have),target=pick(['low','safe','high']);let order;if(target==='low')order=Math.max(5,Math.round((minRaw-Math.max(5,minRaw*.15))/5)*5);else if(target==='high')order=Math.round((maxRaw+Math.max(5,maxRaw*.15))/5)*5;else order=Math.round(randDec(minRaw,maxRaw,1)/5)*5;const safety=order<minRaw?'too low':order>maxRaw?'too high':'safe',ml=finalRound(order*c.qty/c.have),rangeText=perDose?`${c.minDay}–${c.maxDay} mg/kg/dose`:`${c.minDay}–${c.maxDay} mg/kg/day in ${c.doses} equal doses`,safetyLabel=safety==='safe'?'Safe':safety==='too low'?'Too low':'Too high';return q('peds-safe-range','Pediatric safe-dose range',`A child weighs ${kg} kg and is being treated for ${c.context}. The provider orders ${c.drug} ${order} mg ${c.route}. Recommended: ${rangeText}${c.maxDaily?`, maximum ${c.maxDaily} mg/day`:''}. Available: ${c.have} mg/${c.qty} mL. Calculate the volume and determine whether the order is safe, too low, or too high.`,safety,'',perDose?'mg/kg/dose × kg = safe mg/dose range':'mg/kg/day × kg ÷ doses/day = safe mg/dose range','Calculate both ends using unrounded values, classify the ordered dose in mg, then convert both the safe range and ordered dose to mL.',`Safe dose range: ${min}–${max} mg/dose.\nSafe volume range: ${minMl.toFixed(1)}–${maxMl.toFixed(1)} mL/dose.\nOrdered dose: ${order} mg = ${ml.toFixed(1)} mL.\nFinal answer: ${ml.toFixed(1)} mL — ${safetyLabel}.`,.05,{drug:c.drug,context:c.context,kg,order,route:c.route,doses:c.doses,minDay:c.minDay,maxDay:c.maxDay,doseKind:c.doseKind,have:c.have,qty:c.qty,min,max,minRaw,maxRaw,minMl,maxMl,ml,safety,maxDaily:c.maxDaily})}
