(() => {
  ...
  function normalizedSafety(x){
    if(x.safety==='safe'||x.safety==='low'||x.safety==='high')return x.safety;
    const raw=String(x.answer??'').trim().toLowerCase();
    if(raw==='safe')return 'safe';
    if(raw==='too low'||raw==='low')return 'low';
    if(raw==='too high'||raw==='high')return 'high';
    return null;
  }
  function isSafetyQuestion(x){return x.answerMode==='safety'||!!x.safety||x.key==='peds-safe-range'||/pediatric safe-dose range/i.test(x.type||'')}
  function safetyAmount(x){const n=Number(x.amount??x.vars?.ml);return Number.isFinite(n)?n:null}
  function safetyAmountUnit(x){return x.amountUnit||'mL'}
  ...
  function submitSafety(choice){
    if(answered)return;const x=quiz.questions[i],f=app.querySelector('#feedback'),expected=normalizedSafety(x),ok=choice===expected,amount=safetyAmount(x);
    if(ok&&choice==='safe'&&amount!==null){
    ...
    f.innerHTML=`<div class="feedback ${ok?'correct':'incorrect'}"><h3>${ok?'Correct':'Incorrect'}</h3><p>${ok?safetyLabel(expected):`Correct classification: ${safetyLabel(expected)}`}</p>${x.solution?`<div class="solution">${x.solution}</div>`:''}${nextButton()}</div>`;
  }
  function submitSafetyAmount(){
    if(answered)return;const x=quiz.questions[i],input=app.querySelector('#safety-amount'),f=app.querySelector('#feedback'),p=parseAnswer(input?.value||''),amount=safetyAmount(x),unit=safetyAmountUnit(x);
    if(!p){f.insertAdjacentHTML('beforeend','<div class="feedback incorrect" data-amount-error>Enter the amount and unit.</div>');return}
    const expectedUnit=unit.toLowerCase().replace(/\s+/g,''),unitOK=normUnit(p.u)===normUnit(expectedUnit),numOK=amount!==null&&Math.abs(p.n-amount)<=(x.tolerance??.05),ok=unitOK&&numOK;
    answered=true;if(ok)score++;
    f.innerHTML=`<div class="feedback ${ok?'correct':'incorrect'}"><h3>${ok?'Correct':'Incorrect'}</h3><p>${ok?`${amount} ${unit}`:`Correct amount: ${amount} ${unit}`}</p>${x.solution?`<div class="solution">${x.solution}</div>`:''}${nextButton()}</div>`;
  }
  function submit(){
