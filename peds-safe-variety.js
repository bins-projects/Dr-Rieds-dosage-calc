(() => {
  ...
  function makeCase(targetSafety){
    ...
    const orderUnit=unit;
    const prompt=`A child weighs ${kg} kg. The provider orders ${ref.drug} ${order.toLocaleString()} ${orderUnit} IV ${freq} for ${ref.indication}. The recommended dose is ${rangeText}. Available: ${s.have.toLocaleString()} ${s.unit}/${s.qty} mL (${s.note}). Is the ordered dose safe, too low, or too high?`;
  ...
  app.addEventListener('click',e=>{
    ...
    if(choice){
      ...
      const user=choice.dataset.safe,correct=user===active.safety;
      let send;
      if(correct) send=active.orig.safety;
      else send=['safe','low','high'].find(x=>x!==active.orig.safety);
